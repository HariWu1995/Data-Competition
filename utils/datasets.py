...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    def unzip(path):
        # Unzip config.zip TODO: CONSTRAINT: path/to/abc.zip MUST unzip to 'path/to/abc/'
        if str(path).endswith('.zip'):  # path is config.zip
    ...
    with open(check_yaml(yaml_path), errors='ignore') as f:
        data = yaml.safe_load(f)  # config dict
        if zipped:
